...
export const EncryptedText: React.FC<EncryptedTextProps> = ({
  ...
  // FuseOS: deterministic first frame. A random one differs between the server render
  // and the browser's, which React reports as a hydration mismatch (#418).
  const scrambleCharsRef = useRef<string[]>(
    text.split('').map((ch, i) => (ch === ' ' ? ' ' : charset.charAt((i * 7) % charset.length))),
  );
