...
- `website/` — the public download page: Next.js (static export) + Tailwind 4, Aceternity UI components (copied into `src/components/ui/`, recoloured to the ember palette), GSAP ScrollTrigger/SplitText/DrawSVG and Motion. Deployed to GitHub Pages by `.github/workflows/pages.yml`; `pnpm website` or the `website` mprocs pane runs it on :3100. Its Dynamic Island reads `src/lib/island.ts` — call `pushIsland()` to show an event; that is the hook for wiring real FuseOS events in later. Downloads point at the `preview` GitHub release's assets (`FuseOS.dmg`, `FuseOS-preview.apk`). A PNPM workspace.
