...
class LoopGuardTest {
    ...
    fun `a very large sequence number is accepted`() {
        ...
        val guard = guard()
        assertTrue(guard.shouldApply("peer", seq = Long.MAX_VALUE - 1, sentAtUnixMs = 100))
        assertTrue(guard.shouldApply("peer", seq = Long.MAX_VALUE, sentAtUnixMs = 101))
    }
    ...
    fun `sequence numbers are ordered unsigned, matching the wire type`() {
        ...
        assertTrue(guard.shouldApply("peer", seq = 1, sentAtUnixMs = 100))
        assertTrue(guard.shouldApply("peer", seq = Long.MIN_VALUE, sentAtUnixMs = 101))
        // …and having accepted it, ordinary numbers below it are correctly stale.
        assertFalse(guard.shouldApply("peer", seq = 2, sentAtUnixMs = 102))

        // The top of the unsigned range is uint64 max (-1L), above Long.MIN_VALUE.
        assertTrue(guard.shouldApply("peer", seq = -1L, sentAtUnixMs = 103))
        assertFalse(guard.shouldApply("peer", seq = Long.MIN_VALUE, sentAtUnixMs = 104))
    }
    ...
    fun `a gap in sequence numbers does not block later events`() {
        ...
        val guard = guard()
        assertTrue(guard.shouldApply("peer", seq = 1, sentAtUnixMs = 100))
        assertTrue(guard.shouldApply("peer", seq = 9, sentAtUnixMs = 101))
        assertFalse(guard.shouldApply("peer", seq = 5, sentAtUnixMs = 102)) // the late arrival
        assertTrue(guard.shouldApply("peer", seq = 10, sentAtUnixMs = 103))
    }
    ...
    fun `an event from the same millisecond as the last applied one still applies`() {
        ...
        assertTrue(guard.shouldApply("peer", seq = 2, sentAtUnixMs = 500))
    }
    ...
    fun `one millisecond older is rejected`() {
        ...
        assertTrue(guard.shouldApply("peer", seq = 1, sentAtUnixMs = 500))
        guard.recordApplied(LoopGuard.hash("first"), sentAtUnixMs = 500)
        assertFalse(guard.shouldApply("peer", seq = 2, sentAtUnixMs = 499))
    }
    ...
    fun `the timestamp gate only engages once something has been applied`() {
        ...
        // for everyone.
        val guard = guard()
        assertTrue(guard.shouldApply("peer", seq = 1, sentAtUnixMs = 9_000))
        // No recordApplied — e.g. the payload turned out to be empty and was dropped.
        assertTrue(guard.shouldApply("peer", seq = 2, sentAtUnixMs = 5))
    }
    ...
    fun `interleaved events from two peers do not consume each other's sequence numbers`() {
        ...
        val guard = guard()
        assertTrue(guard.shouldApply("peer-a", seq = 1, sentAtUnixMs = 100))
        assertTrue(guard.shouldApply("peer-b", seq = 1, sentAtUnixMs = 101))
        assertTrue(guard.shouldApply("peer-a", seq = 2, sentAtUnixMs = 102))
        assertFalse(guard.shouldApply("peer-b", seq = 1, sentAtUnixMs = 103)) // b's own replay
        assertTrue(guard.shouldApply("peer-b", seq = 2, sentAtUnixMs = 104))
        assertFalse(guard.shouldApply("peer-a", seq = 2, sentAtUnixMs = 105)) // a's own replay
    }
    ...
    fun `last-write-wins is global across peers, so a lagging clock loses`() {
        ...
        val guard = guard()
        assertTrue(guard.shouldApply("fast-clock", seq = 1, sentAtUnixMs = 10_000))
        guard.recordApplied(LoopGuard.hash("from the fast device"), sentAtUnixMs = 10_000)
        assertFalse(guard.shouldApply("slow-clock", seq = 1, sentAtUnixMs = 9_000))
    }
    ...
    fun `an event we originated is refused whatever its sequence number`() {
        ...
        val guard = guard()
        assertFalse(guard.shouldApply("self", seq = 1, sentAtUnixMs = Long.MAX_VALUE))
        assertFalse(guard.shouldApply("self", seq = Long.MAX_VALUE, sentAtUnixMs = 1))
        ...
        assertTrue(guard.shouldApply("selfish", seq = 1, sentAtUnixMs = 1))
    }
